...
# 들어가기 앞서
이번 블로그에서 설명할 내용은 Lustre를 활용하고 있는 환경에서 사용가 새로운 파일을 생성하면, 내부적으로 이루어지 과정을 분석한 내용을 담고 있습니다
 본 내용은 Lustre 전반적인 구조에 대해 기본적으로 알고 있는 것으로 가정하고 기술하기에, Lustre의 구조가 궁금하신 분들은 `Lustre 파일시스템과 GPUDirect Storage 소개`\[1\] 글을 참고하실 수 있습니다.
<br> 또한 Lustre에서 새로운 파일의 생성은 다양한 Component들의 상호작용에 의해 이루어지기 때문에, 총 2편에 걸쳐서 나누어 설명할 예정입니다.
1편에서는 전체적인 흐름을 간략하게 소개하기 위한 Lustre의 software stack, 분석에 활용될 Lustre 디버그 시스템에 대한 소개를 먼저 하고, Lustre client에서
Lustre server로 요청을 보내는 부분까지 서술할 예정입니다. 그리고 이어서 2편에서는 Lustre server에서의 처리 과정과, server에서 처리한 내용을 바탕으로 실제
데이터가 쓰일 OST에 접근하는 과정을 서술할 예정입니다.

## Lustre software stack
Lustre 소프트웨어 스택은 아래 그림과 같이 여러 계층으로 구성되어 있습니다. 해당 그림에서 화살표는 클라이언트에서 Lustre 서버로의 요청 흐름을 나타냅니다.
최초 클라이언트에서 실행된 POSIX  시스템 콜은 `VFS` 계층을 거쳐 Lustre의 `LLITE` 계층으로 이동합니다.
`LLITE`는 해당 요청이 메타데이터에 대한 접근이 필요한 경우 `LMV(Logical Metadata Volume)` 로 요청을 보냅니다. 
이 후 `MDC(Metadata Client)`를 거쳐 서버의 MDS에 요청을 하게 됩니다. `LLITE`에 들어온 요청이 메타 데이터가 아니고 데이터에 대한 요청이라면,
 `LOV(Logical Object Volume)`으로 전달됩니다. `LOV`는 현재 Lustre에 존재하는 클라이언트 수 만큼 `OSC`를 보유하고, 각 `OSC`는 구성된 모든 `OST`에 접근이 가능합니다.

![Alt text](/assets/lustre_software_stack.png)
<center>&#60; Lustre의 전체적인 software stack &#62;</center>

만약 클라이언트에서 파일을 열고 내용을 읽으려고 한다면, 먼저 POSIX를 통해 `OPEN` 시스템 콜이 호출되게 됩니다.
`LLITE`는 해당 파일을 열기 위해서는 `MDS`에 정보를 요청해야 함으로, `LMV`,`MDC`를 통해 `MDS`와 통신해 `Layout EA` 정보를 받아오게 됩니다.
`Layout EA`는 Lustre에서 파일의 `layout`을 관리하기 위한 자료구조로 `Layout Extended Attributes`의 약자입니다. 이 자료구조는 파일을 식별하기 위한 `FID(File Identifiers)` 정보와
어느 `OST`에 존재 하는지에 대한 정보가 존재합니다. `MDS`로 받은 `Layout EA`를 통해 클라이언트는 `LOV`,`OSC`를 거쳐
최종적으로 해당 데이터 블럭이 존재하는 `OST`에 접근해 데이터를 읽거나 쓰게 됩니다.

다음 내용 부터는 Lustre에서 새로운 파일을 생성할 때, 위의 Software stack 에서 어떠한 작업들이 이루어지는지를 추적할 수 있는 방법에 대해 소개한 다음, Client에서부터 과정을 서술할 예정입니다.

## How to lustre debugging
본격적인 내용에 앞서, 해당 내용은 Lustre에서 제공하는 디버그 메세지를 바탕으로 작성되었습니다.
실제 lustre를 구축하고, 본문의 내용을 검증해보자 할 때 활용할 수 있도록, 간단히 소개하고 넘어가도록 하겠습니다.<br>

### debug 설정
Lustre에서는 다양한 디버그 메세지들을 분류해두었기 때문에, 원하는 분류의 메세지만 출력하도록 설정하는 것이 가능합니다.
...
Lustre는 메세지 뿐만 아니라 `subsystem` 별로 디버그 메세지의 분류가 가능합니다.
`subsystem`은 몇 가지를 제외하면 커널 내의 lustre 모듈들을 의미합니다. 
따라서 원하는 모듈만 선택적으로 디버깅이 가능합니다. lustre 에서 지원하는 `subsystem`은 다음과 같습니다.<br>
```text
...
# Client의 최초 처리 과정
Lustre는 `llite` 계층을 활용해 POSIX로 들어온 요청을 lustre에 맞게 변환해서 활용하고 있습니다.
...
일반적인 파일 시스템에 관한 메타데이터 대신 lustre에서 활용하는 메타데이터들을 가지고 요청을 처리합니다.
여러 종류의 메타데이터가 존재하지만,  제일 먼저 활용되는 메타데이터는 앞서 언급된 `FID` 입니다.
`FID`는 lustre에서 파일이나 디렉토리를 식별하기 위한 메타데이터로, 다음과 같이 정의되어 있습니다.<br>
...
sequence 번호는 `MDS`로부터 부여 받고,  obejct id는 동일한 sequence 번호에 새로운 파일이 생성되면 순차적으로 증가하며,
...
```
이 메세지는 `drunk`라는 파일을 열고자 할 때, `ll_atomic_open` 함수에서 기록되는 lustre 디버그 메세지 입니다.
...
단순 파일 열기라면 `IT_OPEN` 플래그만 설정하고, 그게아니라 파일 생성 후에 여는 작업이라면 `O_CREAT` 플래그 값까지 설정합니다.
이 후 수행하는 작업은 크게 2가지 입니다. 첫 번째는 `ll_lookup_it` 이고 두 번째는 `ll_create_it` 입니다.  `ll_lookup_it`는 `MDS`에 해당 파일이
존재하는지 찾고, 없다면 관련 메타 데이터 생성까지 요청한 후, `ll_create_it` 에서 쓸 `layout EA` 메타 데이터를 요청하는 작업까지 수행합니다.
...
`ll_atopmic_open`의 첫 번째 처리 과정인 `ll_lookup_it` 함수를 살펴보기 전에, 먼저 함수 이름에 사용된 `it`에 대해 간단히 설명하고 넘어가도록 하겠습니다.
함수 명의 `it`는 `intent`를 의미합니다. lustre에서는 클라이언트의 요청이 네트워크를 통해 `MDS`로 전달되어 처리가 되기 때문에,
...
`ClstAuto`는 현재 lustre의 클러스터 볼륨명을 의미합니다. `MDT0000` 은 클라이언트의 요청이 `index`가 0인 `MDT`에 전달 되는 것을 의미하고,
`mdc`는 이 요청은 클라이언트의 `mdc`가 보냄을 의미합니다. `drunk` 파일은  `[0x200000bd2:0x3fe:0x0]` 이라는 `FID`를 할당 받았고,
`MDS` 쪽에  이 `FID`에 관해 요청한 것은 `OPEN_INTENT` 입니다. `fid1`은 `drunk` 파일의 부모 디렉토리의 `FID`이고, `fid2`는 `drunk` 자신의 `FID` 입니다.<br>

`MDC`의 요청은 `PTLRPC` 계층을 거쳐 `MDS`로 보내지고, lustre 서버측의 `PTLRPC` 계층의 데몬이 이를 받아서 `MDS` 계층에게 넘겨주게 됩니다.

...
`llc_stripe_offset` 값이 기본값이 아니면 사용자가 offset을 지정한 경우로, 이 경우에는 사용자의 요구 사항에 맞춰 선택됩니다(`lod_ost_alloc_specific`).
...
`ll_create_it`에서는 생성하려는 파일에 대해 메타데이터가 정상적으로 만들어졌기 때문에, `VFS`에 `inode`를 요청합니다. 
